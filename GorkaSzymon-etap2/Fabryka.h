...
#ifndef GORKASZYMON_ETAP2_FABRYKA_H
#define GORKASZYMON_ETAP2_FABRYKA_H
...
#include <iostream>
#include "string"
template<typename T> class Fabryka {

    std::vector<T> pojazd;


public:
    void put(const T& _auto);
    void jazda_probna();
    void dodaj();
    void usun();
    void wypisanie();
...
void Fabryka<T>::dodaj() {
    try{
        T sam;
        std::cout << "Podaj marke model i cene\n";
        std::cin >> sam;
        sam.ladownosc();
        Fabryka::put(sam);
    }
...
#endif //GORKASZYMON_ETAP2_FABRYKA_H
